...
CREATE TABLE Artist (
    ...
    constraint artist_name_un unique(artist_name)
);
...
-- meaning less table, only one foreginkey connect two table.
-- the only purpose is making things worse.
...
CREATE FUNCTION update_movie_title_words() RETURNS trigger AS
$$
begin
  new.title_words := array_agg(distinct lower(w)) from unnest(string_to_array(new.movie_title, ' ')) w;
  return new;
end
$$ language plpgsql;


CREATE TRIGGER update_movie_tsv
       BEFORE INSERT OR UPDATE ON Movie
       FOR EACH ROW EXECUTE PROCEDURE update_movie_title_words();
...
declare
        ...
        artwork_md_id int;
begin
        insert into mediaserver.mediaitem as mi(storage_location)
        values(location)
        returning mi.media_id into media_id;

        insert into mediaserver.metadata as md(md_type_id, md_value)
        select md_type_id, songdescription
        from mediaserver.MetaDataType where md_type_name = 'description'
        returning md.md_id into md_id;

        raise notice 'md_id %', md_id;

        insert into mediaserver.AudioMedia values(media_id);
        ...
        returning md.md_id into genre_md_id;

        insert into mediaserver.MediaItemMetaData
        values (media_id, genre_md_id);

        insert into mediaserver.metadata as md(md_type_id, md_value)
        select md_type_id, artwork
        from mediaserver.MetaDataType where md_type_name = 'artwork'
        returning md.md_id into artwork_md_id;

        insert into mediaserver.MediaItemMetaData
        values (media_id, artwork_md_id);

        artist_id = (select a.artist_id from mediaserver.artist as a
        where lower(a.artist_name) = lower(addSong.artist_name));

        if (artist_id is null) then
           insert into mediaserver.artist as a(artist_name) values(lower(addSong.artist_name))
           returning a.artist_id into artist_id;
        end if;

        insert into mediaserver.Song_Artists
        values(media_id, artist_id);

        insert into mediaserver.MediaItemMetaData
        values (media_id, md_id);

        return media_id; -- song_id is media_id
end;
$$;
...
create or replace function public.total_similarity(
       words varchar[],
       candidate varchar[]
) returns float as
$$
begin
     return sum(similarity(w1, w2))
     from
     unnest(words) w1,
     unnest(candidate) w2;
end;
